...
class LogStash::Filters::CSV < LogStash::Filters::Base
  ...
  # Define whether a file is being parsed and that there are multiple types of files
  # Defaults to false
  config :multiple_file_types, :validate => :boolean, :default => false
  ...
  def filter(event)
    ...
    if (source = event.get(@source))
      begin
        ...
        # Checks if multiple_file_types and autodetect_column_names are true
        if (@multiple_file_types && @autodetect_column_names)
          unless event.get("path").empty?
            if  @file_path != event.get("path")
              @columns = []
            end
          end
        end
